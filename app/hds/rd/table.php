...
final class hds_rd_table
{
  ...
  public function fetch($value, $key = 'id')
  {
    try {
      ...
      $cn = hds_rd::getConn($this->_dbname);
  ...
  public function query($query, $options = null)
  {
    try {
      ...
      if ($query instanceof hds_rd_query) {
        $query = $query->assemble($this->_table);
      }
      
      $sth = hds_rd::getConn($this->_dbname)->query($query);
